    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-slate-100 p-4">
      <Card className="w-full max-w-md shadow-xl border-0">
        <CardHeader className="space-y-2 text-center pb-4">
          ...
          <CardDescription>התחל לנהל את העסק שלך עוד היום</CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="password" type="password" placeholder="לפחות 6 תווים"
                ...
                required minLength={6} className="text-left h-11" dir="ltr" autoComplete="new-password" />
            </div>
          </CardContent>
          <CardFooter className="flex flex-col gap-3 pt-2">
            <Button type="submit" className="w-full h-11 text-base font-semibold" disabled={loading}>
              {loading ? <><Loader2 className="h-4 w-4 animate-spin ml-2" />יוצר חשבון...</> : 'הרשמה'}
            </Button>
            <p className="text-sm text-muted-foreground">
              כבר יש לך חשבון?{' '}
              <Link href="/login" className="text-primary font-semibold hover:underline">כניסה</Link>
            </p>
          </CardFooter>
        </form>
      </Card>
